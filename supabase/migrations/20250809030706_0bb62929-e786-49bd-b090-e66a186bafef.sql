...
-- Create storage policies for produtos (only if not exists)
DROP POLICY IF EXISTS "Produto images are publicly accessible" ON storage.objects;
DROP POLICY IF EXISTS "Admins can upload produto images" ON storage.objects;
DROP POLICY IF EXISTS "Admins can update produto images" ON storage.objects;
DROP POLICY IF EXISTS "Admins can delete produto images" ON storage.objects;
